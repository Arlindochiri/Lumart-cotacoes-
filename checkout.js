...
// ────────────────────────────────────────────────────────────
//  Validação visual em tempo real (check verde quando preenchido)
// ────────────────────────────────────────────────────────────
function ligarValidacaoVisual() {
  // Aplica/remove a classe "preenchido" no .campo conforme o input tem valor
  document.querySelectorAll(".campo input, .campo textarea, .campo select").forEach(input => {
    const campo = input.closest(".campo");
    if (!campo) return;

    const atualizarPreenchido = () => {
      const tem = input.value && input.value.trim() !== "";
      campo.classList.toggle("preenchido", !!tem);
      // Remove erro quando o utilizador começa a corrigir
      if (tem && input.classList.contains("erro")) {
        input.classList.remove("erro");
        campo.classList.remove("erro");
      }
    };

    input.addEventListener("input", atualizarPreenchido);
    input.addEventListener("change", atualizarPreenchido);
    input.addEventListener("blur", atualizarPreenchido);
...
function atualizarTotais() {
  ...
  // Esconder bloco de "restante" quando é 0 (carrinho só com produtos em stock)
  const blocoRestante = document.getElementById("bloco-restante");
  if (blocoRestante) blocoRestante.style.display = restante > 0 ? "" : "none";
...
function validarCheckout() {
  ...
  for (const c of campos) {
    ...
    if (!el || !el.value.trim()) {
      ...
      el?.closest(".campo")?.classList.add("erro");
...
function acaoCheckoutWhatsApp() {
  ...
  const msg =
    ...
    (restante > 0
      ...
      : `  • ${tx("wpp.total_a_pagar", "Total a pagar agora:")} ${formatarMZN(sinal)}`
    ) +
    linhasZAR +
