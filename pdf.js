...
function criarPDFCheckout(dados, numCotacao, salvar = true) {
  ...
  if (dados.pais === "ZA" && typeof TAXA_CAMBIO_ZAR !== "undefined") {
    ...
    doc.text("EQUIVALENTE EM RAND (ZAR)", M + 8, y + 7);

    doc.setFont("helvetica", "normal"); doc.setFontSize(7); doc.setTextColor(107, 103, 120);
    doc.text(
      `Taxa: 1 MZN = ${Number(TAXA_CAMBIO_ZAR.valor).toFixed(4)} R  ·  actualizada em ${dataCambio}`,
      M + 8, y + 13
    );

    // Linhas em 4 colunas: Subtotal | Frete | Total | Sinal
    const colW = (W - M*2 - 16) / 4;
    const colX = (i) => M + 8 + i * colW;

    const cols = [
      ["Subtotal", mznParaZar(subtotal)],
      ["Frete",    mznParaZar(dados.frete)],
      ["Total",    mznParaZar(total)],
      ["Sinal",    mznParaZar(sinal)],
    ];
    ...
    doc.text("Valores em ZAR são referência. Pagamento processado em Metical (MZN).", M + 8, y + 35);
