...
function initProdutoPage() {
  ...
  document.title = `${produtoActual.nome} · Lumart`;
        ...
        <div class="galeria-thumbs" id="galeria-thumbs">
          ${p.imagens.map((img, i) => `
            <button class="thumb ${i === 0 ? 'activa' : ''}" data-i="${i}" onclick="trocarImagem(${i})">
              <img src="${img}" alt="${p.nome} - imagem ${i+1}" />
            </button>
          `).join("")}
        </div>
        <div class="galeria-main">
          <img id="img-principal" src="${p.imagens[0]}" alt="${p.nome}"
...
function renderizarRelacionados() {
  ...
  if (relacionados.length < 4) {
    ...
    const extras = PRODUTOS.filter(p => !ids.has(p.id)).slice(0, 4 - relacionados.length);
