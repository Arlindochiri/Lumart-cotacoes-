...
function initProdutoPage() {
  ...
  // Recently viewed: registar visita (Fase 8B)
  if (typeof registarVisitaProduto === "function") {
    registarVisitaProduto(produtoActual.id);
  }
  ...
  // Recently viewed: render (Fase 8B) — exclui o produto actual
  if (typeof renderizarVistosRecentemente === "function") {
    renderizarVistosRecentemente("secao-vistos-recentemente", {
      limite: 6,
      excluirId: produtoActual.id,
    });
  }
...
function atualizarSEOProduto() {
  ...
  // Meta description — concisa, com nome, marca e início da descrição
  const descCurta = p.descricao ? p.descricao.replace(/\s+/g, " ").substring(0, 145).trim() : "";
  const desc = `${p.nome} — ${p.marca}. ${descCurta}${descCurta.length >= 145 ? "..." : ""} Cotação via WhatsApp em Moçambique e África do Sul.`;
  setMetaContent('meta[name="description"]', desc);

  // Keywords dinâmicas com nome, marca e categoria
  // (meta keywords deprecada — Google ignora-a desde 2009)

  // Canonical URL dinâmica (com ID do produto)
  const canonical = document.getElementById("link-canonical");
  if (canonical) {
    canonical.setAttribute("href", `https://lumartcomercial.com/produto.html?id=${p.id}`);
  }
  ...
  setMetaContent('#og-image-alt', `${p.nome} — ${p.marca} · Lumart Comercial`, "content");
  // Open Graph URL dinâmica
  ...
  ogUrl.setAttribute("content", `https://lumartcomercial.com/produto.html?id=${p.id}`);
  ...
  twTitle.setAttribute("content", `${p.nome} · Lumart Comercial`);
  ...
  twImg.setAttribute("content", p.imagens[0] || "");
  ...
  // Schema.org BreadcrumbList — ajuda Google a mostrar breadcrumbs
  // nos resultados de busca (Inicio › Categoria › Produto)
  let bcSchema = document.getElementById("schema-breadcrumb");
  if (!bcSchema) {
    ...
    "itemListElement": [
      {
        ...
        "position": 1,
        "name": "Início",
        "item": "https://lumartcomercial.com/",
      },
      {
        "@type": "ListItem",
        "position": 2,
        "name": p.categoria,
        "item": `https://lumartcomercial.com/?cat=${encodeURIComponent(p.categoria)}`,
      },
      {
        "@type": "ListItem",
        "position": 3,
        "name": p.nome,
        "item": `https://lumartcomercial.com/produto.html?id=${p.id}`,
      },
    ],
  };
  bcSchema.textContent = JSON.stringify(breadcrumb, null, 2);
}
...
function renderizarBreadcrumb() {
  ...
  const inicio = tFn ? tFn("header.catalogo") : "Catálogo";
  el.innerHTML = `
...
function renderizarProdutoPrincipal() {
  ...
  const precoFinal   = precoComDesconto(p);
  const dispLabel    = p.disponibilidade === "disponivel"
    ? (tFn ? tFn("card.em_stock") : "Em stock")
    : (tFn ? tFn("card.sob_encomenda") : "Sob encomenda");
  ...
  const infoDisp = p.disponibilidade === "disponivel"
    ? {
        titulo: tFn ? tFn("produto.disp_titulo") : "Em stock — envio imediato",
        texto:  tFn ? tFn("produto.disp_texto")  : "Este produto está disponível para envio imediato após confirmação do pagamento.",
        termos: tFn ? tFn("produto.disp_termos") : "Pagamento: 60% no pedido + 40% na entrega",
      }
    : {
        titulo: tFn ? tFn("produto.enc_titulo") : "Sob encomenda — importação directa",
        texto:  tFn ? tFn("produto.enc_texto")  : "Este produto é importado sob encomenda da Ásia ou América. Após confirmação do pagamento de 75%, iniciamos a importação directa para o seu endereço.",
        termos: tFn ? tFn("produto.enc_termos") : "Pagamento: 75% no pedido + 25% na entrega · Prazo: 13–30 dias úteis",
      };
  ...
  document.getElementById("produto-conteudo").innerHTML = `
    <div class="produto-layout">
      ...
      <div class="produto-info reveal">
        ...
        <div class="info-rating">
          ...
          <span class="rating-count">(${numReviews} ${numReviews === 1 ? (tFn ? tFn('produto.avaliacao') : 'avaliação') : (tFn ? tFn('produto.avaliacoes') : 'avaliações')})</span>
        </div>
        ...
        <div class="info-preco-wrap">
          ...
          ${temDesconto ? `<span class="info-poupa">${tFn ? tFn('produto.poupa', { v: formatarMZN(p.preco - precoFinal) }) : `Poupe ${formatarMZN(p.preco - precoFinal)}`}</span>` : ''}
        </div>

        <div class="info-disp">
          <span class="card-badge ${dispClasse}">${dispLabel}</span>
          <button class="btn-partilhar" onclick="partilharProduto()" data-i18n-aria-label="produto.partilhar" aria-label="Partilhar produto">
            <span class="lmi lmi-sm" aria-hidden="true">share</span>
            <span class="btn-partilhar-texto">${tFn ? tFn('produto.partilhar') : 'Partilhar'}</span>
          </button>
        </div>
        ...
        ${Number(p.minimoCotacao) > 0 ? `
          <div class="info-caixa info-caixa-minimo">
            ...
            <div class="caixa-texto">
              <strong>${tFn ? tFn('produto.minimo_titulo', { valor: formatarMZN(p.minimoCotacao) }) : `Cotação mínima: ${formatarMZN(p.minimoCotacao)}`}</strong>
              <p>${tFn ? tFn('produto.minimo_texto', { valor: formatarMZN(p.minimoCotacao) }) : `Este produto só pode ser cotado quando o subtotal do carrinho atingir <strong>${formatarMZN(p.minimoCotacao)}</strong>.`}</p>
            </div>
          </div>
        ` : ''}
        ...
        <div class="info-actions">
          <div class="info-qty">
            <button class="btn-qty" onclick="alterarQtdProduto(-1)" data-i18n-aria-label="comum.remover" aria-label="Diminuir">−</button>
            <span id="qty-actual">1</span>
            <button class="btn-qty" onclick="alterarQtdProduto(1)" data-i18n-aria-label="comum.adicionar" aria-label="Aumentar">+</button>
          </div>
          <button class="btn-add-produto ${emCart ? 'no-carrinho' : ''}" id="btn-add-produto" onclick="adicionarProduto()">
            ${emCart ? (tFn ? tFn('produto.adicionado_cotacao') : '✓ Adicionado à cotação') : (tFn ? tFn('produto.adicionar_cotacao') : 'Adicionar à Cotação')}
          </button>
          <button class="btn-fav-produto ${eFavorito(p.id) ? 'activo' : ''}" data-fav-id="${p.id}"
                  onclick="alternarFavorito(${p.id})"
                  aria-label="${eFavorito(p.id) ? (tFn ? tFn('card.fav_remover') : 'Remover dos favoritos') : (tFn ? tFn('card.fav_adicionar') : 'Adicionar aos favoritos')}"
                  aria-pressed="${eFavorito(p.id)}"
                  title="${tFn ? tFn('card.fav_adicionar') : 'Guardar nos favoritos'}">
            <span class="lmi" aria-hidden="true">${eFavorito(p.id) ? 'favorite' : 'favorite_border'}</span>
          </button>
        </div>
        ...
        <button class="btn-whatsapp-direto" onclick="comprarWhatsAppDirecto()">
          ...
          ${tFn ? tFn('produto.comprar_whatsapp') : 'Comprar directamente via WhatsApp'}
        </button>

        <div class="info-features">
          <div class="feature">
            <span class="lmi lmi-sm" aria-hidden="true">local_shipping</span>
            <span>${tFn ? tFn('produto.feat_entrega') : 'Entrega em todo Moçambique e África do Sul'}</span>
          </div>
          <div class="feature">
            <span class="lmi lmi-sm" aria-hidden="true">schedule</span>
            <span>${tFn ? tFn('produto.feat_prazo') : 'Cotação processada em até 48 horas'}</span>
          </div>
          <div class="feature">
            <span class="lmi lmi-sm" aria-hidden="true">verified_user</span>
            <span>${tFn ? tFn('produto.feat_pagamento') : 'Pagamento seguro · sinal + restante na entrega'}</span>
          </div>
        </div>
    ...
    <div class="produto-sticky-cta" id="produto-sticky-cta" aria-hidden="true">
      ...
      <button class="sticky-btn ${emCart ? 'no-carrinho' : ''}" id="sticky-btn-add" onclick="adicionarProduto()" data-i18n-aria-label="produto.adicionar_cotacao" aria-label="Adicionar à cotação">
        <span class="lmi lmi-sm">add_shopping_cart</span>
        <span class="sticky-btn-txt">${emCart ? t('card.adicionado').replace('✓ ','') : t('comum.adicionar')}</span>
      </button>
    </div>
  `;
...
// ────────────────────────────────────────────────────────────
//  Lightbox — Imagem em ecrã cheio com Pan/Zoom (Fase 8A)
// ────────────────────────────────────────────────────────────
//  Estado do zoom: { scale, tx, ty }
//   - scale: nível de zoom (1 = sem zoom, max 4)
//   - tx, ty: translação (pan) em pixels
//  Inputs:
//   - Mobile: pinch-zoom (2 dedos) + pan (1 dedo quando zoom > 1)
//   - Desktop: scroll-wheel zoom, double-click toggle, drag pan
// ────────────────────────────────────────────────────────────
...
function resetZoom() {
  ...
  aplicarTransform();
  const lb = document.getElementById("lightbox");
  if (lb) lb.classList.remove("a-fazer-zoom");
}
...
function aplicarTransform() {
  ...
  const lb = document.getElementById("lightbox");
  if (lb) lb.classList.toggle("a-fazer-zoom", zoomState.scale > 1.05);
}

// Aplica zoom centrado no ponto (cx, cy) relativo ao centro da imagem
function zoomNoPonto(novaEscala, cx, cy) {
  const img = document.getElementById("lightbox-img");
  if (!img) return;
  novaEscala = Math.max(ZOOM_MIN, Math.min(ZOOM_MAX, novaEscala));

  if (novaEscala === ZOOM_MIN) {
    // Reset
    zoomState = { scale: 1, tx: 0, ty: 0 };
  } else {
    ...
    // Calcular o ponto no espaço da imagem (não escalado)
    const ratio = novaEscala / zoomState.scale;
    // Ajustar translação para manter o ponto sob o cursor fixo
    zoomState.tx = (zoomState.tx - (cx - centerX)) * ratio + (cx - centerX);
    zoomState.ty = (zoomState.ty - (cy - centerY)) * ratio + (cy - centerY);
    zoomState.scale = novaEscala;
    // Limitar pan para não tirar a imagem do ecrã
    limitarPan();
  }
  aplicarTransform();
}
...
function limitarPan() {
  ...
  // Imagem escalada (estimativa): largura/altura natural × scale
  const escalaExcesso = (zoomState.scale - 1) / 2;
  const maxX = rect.width * escalaExcesso;
  const maxY = rect.height * escalaExcesso;
  zoomState.tx = Math.max(-maxX, Math.min(maxX, zoomState.tx));
  zoomState.ty = Math.max(-maxY, Math.min(maxY, zoomState.ty));
}

// ── Listeners (uma vez por sessão) ─────────────────────────
let zoomListenersLigados = false;

function ligarZoomLightbox() {
  if (zoomListenersLigados) return;
  zoomListenersLigados = true;
  ...
  img.addEventListener("wheel", e => {
    ...
    zoomNoPonto(zoomState.scale + delta, e.clientX, e.clientY);
  }, { passive: false });
  ...
  img.addEventListener("dblclick", e => {
    ...
    if (zoomState.scale > 1.05) {
      zoomNoPonto(1, e.clientX, e.clientY);
    } else {
      zoomNoPonto(2.5, e.clientX, e.clientY);
    }
  });
  ...
  // ── Mobile: pinch-zoom (2 dedos) + pan (1 dedo) ─────────────
  ...
  img.addEventListener("touchstart", e => {
    if (e.touches.length === 2) {
      ...
      touchStartTX = zoomState.tx;
      touchStartTY = zoomState.ty;
      touchStartCenterX = (e.touches[0].clientX + e.touches[1].clientX) / 2;
      touchStartCenterY = (e.touches[0].clientY + e.touches[1].clientY) / 2;
    } else if (e.touches.length === 1 && zoomState.scale > 1.05) {
      isTouchPan = true;
      isTouchZoom = false;
      panStartX = e.touches[0].clientX - zoomState.tx;
      panStartY = e.touches[0].clientY - zoomState.ty;
    }
  }, { passive: false });

  img.addEventListener("touchmove", e => {
    if (isTouchZoom && e.touches.length === 2) {
      e.preventDefault();
      const distActual = getDist(e.touches[0], e.touches[1]);
      const novaEscala = touchStartScale * (distActual / touchStartDist);
      zoomNoPonto(novaEscala, touchStartCenterX, touchStartCenterY);
    } else if (isTouchPan && e.touches.length === 1) {
      e.preventDefault();
      zoomState.tx = e.touches[0].clientX - panStartX;
      zoomState.ty = e.touches[0].clientY - panStartY;
      limitarPan();
      aplicarTransform();
    }
  }, { passive: false });
...
// Swipe touch (apenas quando não está em zoom)
let touchStartX = 0;
function onLightboxTouchStart(e) {
  // Não fazer swipe entre imagens se estamos a fazer zoom
  if (zoomState.scale > 1.05) return;
...
// ────────────────────────────────────────────────────────────
//  Partilhar produto (Fase 8D · Web Share API)
// ────────────────────────────────────────────────────────────
async function partilharProduto() {
  if (!produtoActual) return;

  const url = window.location.href;
  const titulo = `${produtoActual.nome} · Lumart Comercial`;
  const texto = `Vê este produto na Lumart Comercial: ${produtoActual.nome} por ${formatarMZN(precoComDesconto(produtoActual))}`;

  // Web Share API (Android Chrome, iOS Safari, desktop Edge/Safari)
  if (navigator.share) {
    try {
...
function renderizarReviews() {
  ...
  seccao.innerHTML = `
    <div class="reviews-header reveal">
      <h2 class="produto-h2">${tFn ? tFn('produto.reviews_titulo') : 'Avaliações dos clientes'}</h2>
      <p class="produto-sub">${reviews.length} ${reviews.length === 1 ? (tFn ? tFn('produto.avaliacao') : 'avaliação real') : (tFn ? tFn('produto.avaliacoes') : 'avaliações reais')}</p>
    </div>
    ...
    <div class="reviews-layout reveal">
      <div class="reviews-resumo">
        ...
        <div class="resumo-total">${reviews.length} ${reviews.length === 1 ? (tFn ? tFn('produto.avaliacao') : 'avaliação') : (tFn ? tFn('produto.avaliacoes') : 'avaliações')}</div>
...
function renderizarRelacionados() {
  ...
  const txtAdd     = tFn ? tFn("card.adicionar") : "Adicionar";

  seccao.innerHTML = `
    <div class="relacionados-header reveal">
      <h2 class="produto-h2">${tFn ? tFn("produto.relacionados_titulo") : "Pode também gostar"}</h2>
      <a href="index.html" class="ver-todos">${tFn ? tFn("produto.ver_catalogo_todo") : "Ver todo o catálogo →"}</a>
    </div>
    <div class="produtos-grid reveal">
      ${relacionados.map(p => {
        ...
        const temDesconto = p.desconto && p.desconto > 0;
        const precoFinal  = precoComDesconto(p);
        const dispLabel   = p.disponibilidade === "disponivel" ? txtEmStock : txtSobEnc;
        const dispClasse  = p.disponibilidade === "disponivel" ? "disp-stock" : "disp-encomenda";
          ...
          <article class="card-produto">
            ...
            <div class="card-corpo">
              ...
              <div class="card-footer">
                ...
                <button class="btn-add-mobile ${emCart ? 'no-carrinho' : ''}" data-id="${p.id}" onclick="adicionarAoCarrinho(${p.id})" aria-label="${txtAdd}">
                  ${emCart ? '✓' : '+'}
                </button>
              </div>
            </div>
