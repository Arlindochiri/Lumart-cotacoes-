...
//    minimoCotacao   — valor mínimo total da cotação (em MZN) para incluir
//                      este produto. 0 ou ausente = sem mínimo. Regra de
//                      negócio para produtos importados que não compensam
//                      vender abaixo de certo total.
...
//    ativo           — true (visível) | false (pausado, escondido do site público)
...
// ════════════════════════════════════════════════════════════
//  3.1 TAXA DE CÂMBIO MZN → ZAR
// ════════════════════════════════════════════════════════════
//  Usado quando o cliente escolhe África do Sul como destino.
//  Cálculo: precoZAR = precoMZN × TAXA_CAMBIO_ZAR
//
//  IMPORTANTE: o preço base de todos os produtos continua em MZN.
//  A conversão para ZAR é apenas visual (cliente ZA vê equivalente).
//
//  Edição: pelo painel admin (Tab "Configurações → Taxa de Câmbio")
//  ou directamente neste ficheiro.
// ════════════════════════════════════════════════════════════
const TAXA_CAMBIO_ZAR = {
  valor: 0.27,                    // 1 MZN = 0.27 ZAR (placeholder — actualizar)
  dataActualizacao: "2026-05-11", // YYYY-MM-DD
};
...
const PAGAMENTO_REGRAS = {
  disponivel:    { sinal: 1.00, restante: 0.00 }, // 100% no pedido (em stock)
  sob_encomenda: { sinal: 0.75, restante: 0.25 }, // 75% / 25% (sob encomenda)
};
...
/**
 * Converte um valor de MZN para ZAR usando a taxa configurada.
 * Ex: 1000 → 270 (com TAXA_CAMBIO_ZAR.valor = 0.27)
 */
...
/**
 * Formata um valor numérico em ZAR (Rand sul-africano).
 * Ex: 270.5 → "R 270,50"
 */
...
/**
 * Formata um valor MZN com equivalência ZAR ao lado (em itálico).
 * Útil para checkout/PDF quando o cliente escolhe África do Sul.
 * Ex: 1000 → "MZN 1.000,00 (≈ R 270,00)"
 */
...
/**
 * Calcula o sinal a pagar agora (produtos em stock: 100% no pedido;
...
function termosPagamentoTexto(carrinho) {
  ...
  if (temDisponivel && temSobEnc) {
    return [
      tFn ? tFn("checkout.termos_em_stock") : "Produtos em stock: 100% no pedido",
      tFn ? tFn("checkout.termos_sob_enc")  : "Produtos sob encomenda: 75% no pedido + 25% na entrega",
    ];
  }
  if (temSobEnc) {
    return [tFn ? tFn("checkout.termos_so_enc") : "75% no momento do pedido + 25% na entrega"];
  }
  return [tFn ? tFn("checkout.termos_so_stock") : "100% no momento do pedido"];
}
...
/**
 * Devolve apenas os produtos activos (visíveis ao público).
 * Produtos com `ativo: false` ficam escondidos do site mas mantêm-se
 * no ficheiro (gestão feita pelo painel de admin).
 */
...
//  produtos com mínimo no carrinho, o subtotal total dos produtos
//  (excluindo frete) tem de ser >= ao maior mínimo entre eles.
//
//  Exemplo: Estojo W&N tem minimoCotacao=2000.
//           Se cliente põe só 1 estojo (290 MT) → bloqueado, faltam 1710 MT.
//           Cliente pode: aumentar quantidade OU adicionar outros produtos.
//
//  Esta regra aplica-se na geração da cotação (PDF + WhatsApp).
//  O frete NÃO conta para o mínimo (só o subtotal de produtos).
// ════════════════════════════════════════════════════════════

/**
 * Valida se o carrinho cumpre os mínimos de cotação dos produtos.
 *
 * @param {Array} carrinho — array de itens
 * @returns {Object} {
 *   valido: boolean,             — true se passa
 *   subtotal: number,            — subtotal actual em MZN
 *   maiorMinimo: number,         — maior mínimo aplicável (0 = sem mínimo)
 *   produtoLimitante: Object|null, — produto que define o maior mínimo
 *   produtosComMinimo: Array,    — todos os produtos com minimoCotacao > 0
 *   faltam: number               — 0 se válido; quanto falta para atingir
 * }
 */
function validarMinimoCotacao(carrinho) {
  ...
  // Filtra produtos com mínimo declarado > 0
  ...
  // Produto com o maior mínimo é o limitante
  ...
  return {
    valido, subtotal, maiorMinimo,
    produtoLimitante, produtosComMinimo, faltam,
  };
}
